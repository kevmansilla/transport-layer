...
# Cargo base
data = pandas.read_csv('caso2_2.csv')
print(data)
print(data.columns)
...
ax1 = plt.subplot(212)
plt.suptitle("Caso 2 (generationInterval = 1): Ocupacion de buffers en el sistema")
...
plt.suptitle("Caso 2 (generationInterval = 1): Paquetes enviados y recibidos")
sns.barplot(
    x=['Enviados', 'Recibidos'],
    y=[len(packets_gen), len(packets_sink)])
print(len(packets_gen)) #200
print(len(packets_sink)) #200
plt.show()
